...
Launcher::Launcher(bool firstShow, QQuickView *w)
    : QQuickView(w)
    , m_dockInterface("com.long.Dock",
                    "/Dock",
                    "com.long.Dock", QDBusConnection::sessionBus())
    ...
    , m_bottomMargin(0)
{
    ...
    if (m_dockInterface.isValid() && !m_dockInterface.lastError().isValid()) {
        ...
        QDBusServiceWatcher *watcher = new QDBusServiceWatcher("com.long.Dock",
...
{
    QDBusInterface iface("com.long.Dock",
                         "/Dock",
                         "com.long.Dock",
                         QDBusConnection::sessionBus());
