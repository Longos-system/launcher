...
LauncherModel::LauncherModel(QObject *parent)
    ...
    , m_settings("longos", "launcher-applist", this)
    , m_mode(NormalMode)
    , m_firstLoad(false)
{
    // Init datas.
...
void LauncherModel::sendToDock(const QString &key)
{
    ...
    if (index != -1) {
        QDBusMessage message = QDBusMessage::createMethodCall("com.long.Dock",
                                                              "/Dock",
                                                              "com.long.Dock",
                                                               "add");
...
bool LauncherModel::launch(const QString &path)
{
    ...
    if (index != -1) {
        ...
        // long-screenshot needs to be processed.
        if (cmd == "long-screenshot") {
